...
contract('Permissions', ([patient, serviceProvider, dev]) => {
  ...
  describe('add serivce provider and new emr', async () => {
      ...
      serviceProvidersRevoke, beforeRemove
      before(async () => {    
        const time = new Date()
        ...
        await permissions.giveAccessToServiceProvider(serviceProvider, { from: patient })
        serviceProvidersAfter = await permissions.getServiceProviderPermissions( { from: patient })
        await permissions.addEmr(patient,1,0,time.toISOString(), "testing",{from:serviceProvider})
        ...
        await permissions.revokeAccessFromDoctor(serviceProvider,{from: patient})
